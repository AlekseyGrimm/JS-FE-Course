...
// Choose 3 sites and analyze it using DevTools console:
// => Do they have memory leaks?
// => What functions are taking more time for execution?
// => What time does it take for rendering first meaningful pag

// Все анализы проводились с настройками в Perfomance сеть 3g, понижение производительности процессора x4.

// 1)https://www.21vek.by/

// => да, имеются утечка памяти во время Parse HTML. Long task 152.61ms (self 0.69ms)
// Также имеется проблемы с выполнением JS кода, а именно self 0.49ms (long task 76.74ms)
// Кроме того, имеются множественный запросы (400), в том числе на js файлы, Дом компилится за 17.74s,
// а общая загрузка, отрисовка, обработка и ожидания нерабочих запросов занимает  42,70s.

// => DOMContentLoaded - Long task.

// => 6s (согласно сведений вкладки performance)


// 2)https://mail.ru/

// => Согласно вкладки Performance имеется утечка памяти выполнение нерабочих запросов картинок
// и файлов js. Также имеется 6 нерабочих запросов на сторонние ресурсы, которые также
// также потребляют ресурсы проихводительности на ожидание.

// => function 'Timer Fired' - Long task(1.48s).

// => 4s

//3)https://www.onliner.by/

// => имеются запросы на сторонние ресурсы, которые не работают (утечка памяти, так как нету смысла их использовать если они не нужны)
// выполнение кода занимает 7,36s из-за ожидания нерабочих запросов. 
